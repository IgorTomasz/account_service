...
WORKDIR /src
COPY ["account_service.csproj", "."]
RUN dotnet restore "./account_service.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./account_service.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./account_service.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
